// \command completion: the bundled CTAN signature DB (same one the parser uses), a small extras
// table, and the user's OWN \newcommand/\NewDocumentCommand definitions scanned live from the
// buffer (LaTeX Workshop calls this "user-defined macros"; here it's listNewcommands reused from
// the parser's own converter.ts, not a bespoke scanner).
...
import { macroCompletion } from './shared';
...
export const STATIC_MACRO_OPTIONS: Completion[] = [...STATIC_SIGNATURES].map(([name, sig]) => macroCompletion(name, sig));
...
function computeUserMacros(text: string): Completion[] {
	...
	try {
		...
		for (const m of listNewcommands(ast)) {
			...
			if (STATIC_NAMES.has(m.name) || seen.has(m.name)) continue;
			seen.add(m.name);
