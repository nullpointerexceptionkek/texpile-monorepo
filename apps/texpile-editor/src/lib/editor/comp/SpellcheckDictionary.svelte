	...
	>
		<div class="card bg-surface-50-950 border-surface-300-700 flex max-h-[80vh] w-full max-w-md flex-col border p-4 shadow-2xl">
			<div class="mb-3 flex items-center justify-between">
				<h2 class="text-base font-semibold">Custom dictionary</h2>
				<button class="btn-icon btn-icon-sm hover:preset-tonal" type="button" aria-label="Close" onclick={() => (open = false)}>
					<X class="size-4" />
				</button>
			</div>
			...
			<div class="mb-3 flex gap-2">
				...
				/>
				<button class="btn btn-sm preset-filled-primary-500" type="button" onclick={add} disabled={!newWord.trim()}>Add</button>
			</div>
			...
			<ul class="min-h-0 flex-1 overflow-y-auto">
				...
				{:else}
					<li class="text-surface-500 py-4 text-center text-sm">No custom words yet.</li>
				{/each}
			</ul>
		</div>
	</div>
